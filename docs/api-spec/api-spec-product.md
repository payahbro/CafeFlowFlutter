...
### Table `products`
- `id` UUID PK
- `name` VARCHAR(100), unique lower(name)
...
### Transaction Rules
- Mutasi (`POST`, `PUT`, `PATCH status`, `DELETE`, `RESTORE`) wajib DB transaction
- Soft delete policy wajib (tidak hard delete)
...
Wajib log:
- Operasi mutasi + `user_id`, `role`, `product_id`
- Redis down/timeout (WARNING)
