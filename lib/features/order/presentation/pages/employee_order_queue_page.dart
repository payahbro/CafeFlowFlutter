...
class EmployeeOrderQueuePage extends StatefulWidget {
  const EmployeeOrderQueuePage({
    super.key,
  ...
  @override
  State<EmployeeOrderQueuePage> createState() => _EmployeeOrderQueuePageState();
}
...
class _EmployeeOrderQueuePageState extends State<EmployeeOrderQueuePage> {
  ...
  Widget? _buildQuickAction(String orderId) {
    final state = _controller.state;
    ...
    if (order.status != OrderStatus.confirmed) {
      return Text(
        order.status == OrderStatus.pending
            ...
            : 'Tidak ada aksi cepat untuk status ini',
    ...
    return SizedBox(
      ...
      child: ElevatedButton.icon(
        onPressed: state.isOrderBusy(orderId)
            ? null
  ...
  Future<void> _markAsCompleted(String orderId) async {
    ...
    try {
      ...
      messenger.showSnackBar(
        const SnackBar(content: Text('Status pesanan diperbarui ke COMPLETED')),
      );
    } catch (error) {
      if (!mounted) {
        return;
